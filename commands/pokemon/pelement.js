...
const { ELEMENT_TYPES, getElementLabel } = require("../../services/elementType");

module.exports = {
  name: "pelement",
  async execute({ event, say }) {
    const elements = ELEMENT_TYPES;
    const rows = [];
    for (let i = 0; i < elements.length; i += 5) {
      rows.push({
        ...
        elements: elements.slice(i, i + 5).map((element) => ({
          ...
          text: { type: "plain_text", text: getElementLabel(element), emoji: true },
