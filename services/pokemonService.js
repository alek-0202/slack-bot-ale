...
async function insertUserPokemon({
  ...
  const { data, error } = await supabase
    ...
    .select("id, species_id, level, shiny, shiny_type, attack_iv, magic_iv, defense_iv, hp_iv, speed_iv, crit_level, dodge_level, elemental_level, attack, magic, defense, hp, current_hp, speed, source, captured_at, upgrade_spent_gold, book_bonus_attack, book_bonus_magic, book_bonus_defense, book_bonus_hp, book_bonus_speed, is_battle_available, is_favorite, legendary_passive_id, legendary_passive_code, legendary_passive_values, legendary_passive_efficiency, epic_affix_type, epic_affix_value, epic_affix_label, epic_affix_value_type, epic_affix_metadata, epic_affix_updated_at")
    .single();
...
async function getUserPokemonById(slackUserId, pokemonId) {
  ...
  const { data, error } = await supabase
    .from("user_pokemons")
    .select(
      "id, slack_user_id, species_id, level, shiny, shiny_type, crit_level, dodge_level, elemental_level, attack, magic, defense, hp, current_hp, speed, upgrade_spent_gold, book_bonus_attack, book_bonus_magic, book_bonus_defense, book_bonus_hp, book_bonus_speed, is_battle_available, is_favorite, legendary_passive_id, legendary_passive_code, legendary_passive_values, legendary_passive_efficiency, epic_affix_type, epic_affix_value, epic_affix_label, epic_affix_value_type, epic_affix_metadata, epic_affix_updated_at, pokemon_species(id, name, rarity, base_value, sprite_url, element_types, evolves_to, evolution_stage, base_attack, base_magic, base_defense, base_hp, base_speed)",
    )
    .eq("id", pokemonId)
    .eq("slack_user_id", slackUserId)
    .maybeSingle();
...
async function setPokemonFavorite(slackUserId, pokemonId, isFavorite) {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", pokemonId)
    .eq("slack_user_id", slackUserId)
    .select("id, is_favorite, legendary_passive_id, legendary_passive_code, legendary_passive_values, legendary_passive_efficiency, epic_affix_type, epic_affix_value, epic_affix_label, epic_affix_value_type, epic_affix_metadata, epic_affix_updated_at")
    .maybeSingle();
