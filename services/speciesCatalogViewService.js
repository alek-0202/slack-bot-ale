...
async function getAllSpeciesCatalog() {
  ...
  const { data, error } = await supabase
    ...
    .select("id, name, generation, sprite_url, rarity, evolution_stage, evolves_from, evolves_to, base_value, element_types, base_attack, base_magic, base_defense, base_hp, base_speed")
...
function buildSpeciesMessage({ slackUserId, entry, index, total, speciesIds = null }) {
  ...
  const detailsText =
    ...
    `📊 Base: ⚔️ *${entry.base_attack || 0}* | ✨ *${entry.base_magic ?? entry.base_attack ?? 0}* | 🛡️ *${entry.base_defense || 0}* | ❤️ *${entry.base_hp || 0}* | 💨 *${entry.base_speed || 0}*\n` +
