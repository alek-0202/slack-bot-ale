...
async function getOwnedPokemonById(pokemonId) {
  ...
  const { data, error } = await supabase
    .from("user_pokemons")
    .select(
      "id, slack_user_id, species_id, level, shiny, attack, defense, hp, speed, captured_at, pokemon_species(id, name, rarity, sprite_url, evolves_to, evolution_stage, element_types)",
    )
    .eq("id", pokemonId)
    .maybeSingle();
...
async function findUserPokemonsBySpeciesName({ slackUserId, speciesName }) {
  ...
  const { data, error } = await supabase
    .from("user_pokemons")
    .select(
      "id, slack_user_id, species_id, level, shiny, attack, defense, hp, speed, captured_at, pokemon_species!inner(id, name, rarity, sprite_url)",
    )
