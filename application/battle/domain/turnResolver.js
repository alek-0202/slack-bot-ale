...
function buildResolvedActionPayload({
  ...
  const normalizedDodged = Boolean(didDodge || wasDodged || dodged);
  return {
    actorId: actorUserId,
    ...
    didDodge: normalizedDodged,
    wasDodged: normalizedDodged,
    isCrit: Boolean(isCrit),
    critMultiplier: Math.max(1, Number(critMultiplier) || 1),
    ...
    elementalModifier: Math.max(0, Number(elementalModifier) || 0),
    elementalMultiplier: Math.max(0, Number(elementalMultiplier) || 0),
    elementalOutcome: elementalOutcome || elementalRelation || "neutral",
    elementalRelation,
    dodged: normalizedDodged,
...
function resolveBattleTurn({ battle, actorUserId, actionType, actionPayload = {} }) {
  ...
  if (actionType === BATTLE_ACTION.MAGIC) {
    ...
    if (magicAction.kind === "regular") {
      ...
      const resolvedAction = buildResolvedActionPayload({
        ...
        didDodge: Boolean(result.dodged),
        wasDodged: Boolean(result.dodged),
        isCrit: Boolean(result.isCritical),
        critMultiplier: Number(result.critMultiplier || 1),
        ...
        elementalModifier: Number(result?.elementalModifier || result?.multiplier || 1),
        elementalMultiplier: Number(result?.multiplier || 1),
        elementalOutcome: result?.elementalOutcome || result?.elemental?.relation || "neutral",
      ...
      logBattleDebug("resolved_magic_regular", {
        ...
        didDodge: result.dodged,
        targetUserId: defenderId,
        actionKind: regularMagic?.element || "regular-magic",
        elementalModifier: result?.elementalModifier || result?.multiplier || 1,
        elementalOutcome: result?.elementalOutcome || result?.elemental?.relation || "neutral",
    ...
    const resolvedAction = buildResolvedActionPayload({
      ...
      critMultiplier: Number(castResult.critMultiplier || 1),
      ...
      elementalModifier: Number(resolveElementalDamageRule({ attackElement: magicAction.element, defenderElements: defender.selectedPokemon?.elementTypes || [] }).multiplier || 1),
      ...
      elementalOutcome: resolveElementalDamageRule({ attackElement: magicAction.element, defenderElements: defender.selectedPokemon?.elementTypes || [] }).relation || "neutral",
  ...
  if (actionType === BATTLE_ACTION.ATTACK) {
    ...
    const resolvedAction = buildResolvedActionPayload({
      ...
      finalDamage: Number(damageWithHooks.finalDamage || 0),
      shieldAbsorbedDamage: Number(damageWithHooks.shieldAbsorbedDamage || 0),
      elementalModifier: Number(result?.elementalModifier || result?.elemental?.multiplier || 1),
    ...
    logBattleDebug("resolved_attack", {
      ...
      dodgeRoll: result.dodgeRoll,
      targetUserId: defenderId,
      actionKind: "basic-attack",
      baseDamage: result.normalDamage,
      finalDamage: damageWithHooks.finalDamage,
      shieldAbsorbedDamage: damageWithHooks.shieldAbsorbedDamage,
      elementalModifier: result?.elementalModifier || result?.elemental?.multiplier || 1,
      elementalOutcome: result?.elementalOutcome || result?.elemental?.relation || "neutral",
      elementalRelation: result?.elemental?.elemental?.relation || "neutral",
