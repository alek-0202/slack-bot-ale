alter table public.pokemon_species add column if not exists base_magic integer;
update public.pokemon_species
set base_magic = coalesce(base_magic, base_attack, 10)
where base_magic is null;
alter table public.pokemon_species alter column base_magic set default 10;
alter table public.pokemon_species alter column base_magic set not null;

alter table public.user_pokemons add column if not exists magic integer;
update public.user_pokemons
set magic = coalesce(magic, attack, 10)
where magic is null;
alter table public.user_pokemons alter column magic set default 10;
alter table public.user_pokemons alter column magic set not null;
...
declare
  ...
  v_magic integer := greatest(coalesce(p_base_magic, p_base_attack, 10), 1);
...
begin
  ...
  select greatest(round(v_attack * (1 + (0.18 * v_level_gains) + (0.25 * v_milestones)))::integer + case when v_level = 50 then 5 else 0 end, 1),
         greatest(round(v_magic * (1 + (0.19 * v_level_gains) + (0.26 * v_milestones)))::integer + case when v_level = 50 then 6 else 0 end, 1),
...
declare
  ...
  v_speed integer;
begin
  select up.level, coalesce(up.upgrade_spent_gold, 0), ps.base_attack, ps.base_magic, ps.base_defense, ps.base_hp, ps.base_speed
    into v_level, v_refund, v_base_attack, v_base_magic, v_base_defense, v_base_hp, v_base_speed
  from public.user_pokemons up
  join public.pokemon_species ps on ps.id = up.species_id
  where up.id = p_pokemon_id
    and up.slack_user_id = p_slack_user_id
...
begin
  ...
  select dm.species_id, dm.price, ps.rarity, ps.base_attack, ps.base_magic, ps.base_defense, ps.base_hp, ps.base_speed
    into v_species_id, v_price, v_rarity, v_base_attack, v_base_magic, v_base_defense, v_base_hp, v_base_speed
  ...
  exception
    when unique_violation then
      delete from public.user_pokemons where id = user_pokemon_id;
      update public.users set gold = gold + v_price where slack_user_id = p_slack_user_id;
      delete from public.transactions where slack_user_id = p_slack_user_id and type = 'market_purchase' and amount = -v_price;
      return query select false, 'already_bought_slot', v_species_id, v_price, v_user_gold, null::bigint;
      return;
  end;
