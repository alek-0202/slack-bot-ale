-- Incremental fix: re-apply HP ratio preservation logic without relying on prior migration edits.
...
begin
  ...
  return least(
    v_new_max_hp,
    greatest(0, floor((v_current_hp::numeric / v_old_max_hp::numeric) * v_new_max_hp)::integer)
  );
end;
$$;
...
-- Defensive data cleanup for historical rows with out-of-range HP values.
update public.user_pokemons
set current_hp = least(greatest(coalesce(current_hp, 0), 0), greatest(coalesce(hp, 0), 0))
where coalesce(current_hp, 0) < 0
   or coalesce(current_hp, 0) > greatest(coalesce(hp, 0), 0);
