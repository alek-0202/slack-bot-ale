...
test('calculateShinyTransferGoldCost escala por diferença positiva de raridade', () => {
  ...
  assert.equal(service.calculateShinyTransferGoldCost({ sourceRarity: 'rare', targetRarity: 'rare' }), 5000000);
  assert.equal(service.calculateShinyTransferGoldCost({ sourceRarity: 'rare', targetRarity: 'epic' }), 10000000);
  assert.equal(service.calculateShinyTransferGoldCost({ sourceRarity: 'common', targetRarity: 'epic' }), 20000000);
});
...
test('getShinyTransferPreview calcula custo e valida saldo insuficiente com custo correto', async () => {
        ...
        return {
          select() {
            return {
              eq() {
                return {
                  maybeSingle: async () => ({ data: { gold: 14000000 }, error: null }),
                };
              },
            };
          },
  ...
  assert.equal(result.costGold, 15000000);
});
